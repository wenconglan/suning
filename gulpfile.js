...
//1.gulp复制
gulp.task('copyfont', () => {
    return gulp.src('src/font/*') //加载文件
        .pipe(gulp.dest('dist/font/'));//输出文件。
});
gulp.task('copythirdplugs', () => {
    return gulp.src('src/thirdplugs/*') //加载文件
        .pipe(gulp.dest('dist/thirdplugs/'));//输出文件。
});
...
gulp.task('uglifyimg', () => {
    return gulp.src('src/imgs/*.{png,gif,jpg,ico}')
        .pipe(imagemin())//执行压缩
...
gulp.task('default', function () {
    watch(['src/html/*.html', 'src/css/*.css', 'src/js/*.js', 'src/imgs/*.{png,gif,jpg,ico}'], gulp.parallel('uglifyhtml', 'uglifycss', 'uglifyjs', 'uglifyimg'));
});
